...
## FinReview Financial Product Components (Clay Aesthetic)

Adapts Clay's warm ivory canvas, charcoal typography, pill geometry, and vibrant saturated accents for the FinReview AI-native financial review application.

### 1. Ingestion Dropzone & Status
- **Canvas**: `{colors.surface-card}` (`#f5f0e0`) with 2px dashed `{colors.hairline}` border and `{rounded.xl}` (24px).
- **CTA**: Solid dark `{colors.primary}` (`#0a0a0a`) with `{colors.on-primary}` text and `{rounded.md}` (12px).
- **Progress / Processing State**: Animated pulse on warm `{colors.surface-soft}` banner with Clay-style processing status pill.

### 2. Transaction Table (Data-Dense & Scannable)
- **Container**: White / Ivory `{colors.surface-card}` card with `{colors.hairline}` borders and `{rounded.lg}` (16px).
- **Header**: Sticky bar with `{colors.muted}` 12px uppercase tracking, crisp hairline bottom divider.
- **Rows**: Alternating subtle warmth; hover state highlights with `{colors.surface-soft}`.
- **Amounts**: Monospace or tabular figures (`font-mono`) aligned right, with positive amounts in `{colors.body-strong}` and negative amounts with crisp formatting.

### 3. Category & Confidence Badges
- **Pills**: Height 24px, `{rounded.pill}`, 12px font-weight 600.
  - **Revenue**: `{colors.brand-mint}` background (`#a4d4c5`) with dark `{colors.ink}` text.
  - **COGS**: `{colors.brand-peach}` background (`#ffb084`) with dark `{colors.ink}` text.
  - **Payroll**: `{colors.brand-lavender}` background (`#b8a4ed`) with dark `{colors.ink}` text.
  - **Operating Expenses**: `{colors.brand-ochre}` background (`#e8b94a`) with dark `{colors.ink}` text.
  - **Needs Review (Uncertain)**: `{colors.warning}` amber dot + pale amber badge with inline quick-correct trigger.

### 4. Deterministic P&L Statement Grid
- **Columns**: Line Item, followed by consecutive Month columns (e.g. Jan, Feb, Mar) and MoM Variance (%).
- **Hierarchy**:
  - Top line: **Revenue** (bold, Plain Black/Inter 600)
  - Subtractions: **Cost of Goods Sold** (COGS)
  - Subtotal: **Gross Profit** (highlighted with subtle `{colors.surface-strong}` tint)
  - Expenses: **Payroll**, **Operating Expenses**
  - Final: **Operating Profit** (double underline or accented border)
- **Integrity Rule**: All figures computed deterministically from verified transaction records. Zero LLM involvement in arithmetic.

### 5. Variance Cards & Driver Explanations
- **Card**: `{colors.surface-card}` with subtle border and `{rounded.md}`.
- **Delta Indicator**:
  - **Favorable Change**: Pill in `{colors.success}` (`#22c55e`) with `+` delta and percentage.
  - **Unfavorable Change**: Pill in `{colors.error}` (`#ef4444`) with `-` delta and percentage.
- **Driver Breakdown**: List of driving transaction groups with immediate drill-down links to view exact contributing rows.
- **AI Narrative Block**: Clean callout card with `{colors.brand-lavender}` accent bar summarizing reasons with linked data references.

### 6. Review Queue & Correction Modal
- **Queue Panel**: Card stack of flagged/uncertain transactions requiring human judgment.
- **Action Buttons**:
  - Primary Confirm: Dark button `{colors.primary}`.
  - Inline Edit/Reclassify: Category selector dropdown with immediate persistence and audit log entry.

### 7. AI Financial Analyst Drawer with Traceable Citations
- **Layout**: Persistent split-screen or slide-over drawer on `{colors.canvas}`.
- **Prompt Input**: Pill or rounded-md input with clear example prompts ("What was our revenue in March?", "Why did operating profit drop?").
- **Message Bubbles**:
  - User: Dark `{colors.primary}` pill card.
  - Analyst: Warm white `{colors.surface-card}` card with markdown formatting.
- **Citation Chips**: Saturated mini-pills (`#b8a4ed` or `#faf5e8` with hairline border) citing specific `[Txn #ID]` or `[Category: Payroll]`. Clicking any chip scrolls and highlights the transaction in the table.
